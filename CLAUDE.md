...
| `scaffold-video.js` | `node tools/scaffold-video.js <idGuía>` genera en `videos/<slug>/` un proyecto de vídeo TikTok (HyperFrames faceless-explainer) con storyboard+guion+diseño ya rellenos desde la guía (gancho + N puntos + CTA, estilo "producto 3D en pista de noche / motion comic" aprobado el 23/09/2026, voz Fernando Sanz; copia al proyecto la guía de estilo, la referencia, la plantilla de portada `cover.html` y la foto real de cada producto sin fondo, recortada con `tools/video-assets/recortar-fondo.py`; todo versionado en `tools/video-assets/`, y el lima/negro anterior en `frame-blockframe.md`; HyperFrames fijado a `@0.8.63`). `--list` lista guías. NO produce el MP4 solo: los frames los construye un agente (workers HyperFrames); el script imprime los pasos restantes (voz→frames→ensamblar→render). El borrador de gancho/VO hay que repasarlo. `videos/` está en `.gitignore`. Ver [[tiktok-video-pipeline]] en memoria. |
